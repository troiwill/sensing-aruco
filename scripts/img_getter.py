...
def calculate_depth(data):
    ...
    # print(np.min(roi_bounds[:, 0]), np.max(roi_bounds[:, 0]), depth_image.shape[0], np.min(roi_bounds[:, 1]), np.max(roi_bounds[:, 1]), depth_image.shape[1])
...
def node():
    ...
    #rospy.Subscriber("/camera/color/image_raw", Image, calculate_pose)
    #rospy.Subscriber("/camera/depth/image_raw", Image, calculate_depth)
